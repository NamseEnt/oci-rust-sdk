use serde::{Deserialize, Serialize};

#[allow(unused_imports)]
use super::*;
#[allow(unused_imports)]
use super::super::models::*;

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AttachInstancePoolInstanceResponse {
    /// Unique Oracle-assigned identifier for the request. If you need to contact Oracle about a particular request, please provide the request ID.
    pub opc_request_id: String,

    /// For optimistic concurrency control. See {@code if-match}.
    pub etag: String,

    /// The [OCID](https://docs.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the work request. Use [GetWorkRequest](https://docs.oracle.com/iaas/api/#/en/workrequests/latest/WorkRequest/GetWorkRequest) with this ID to track the status of the request.
    pub opc_work_request_id: String,

    /// Location of the resource.
    pub location: String,

    /// The returned model.InstancePoolInstance instance.
    pub instance_pool_instance: InstancePoolInstance,
}


/// Required fields for AttachInstancePoolInstanceResponse
pub struct AttachInstancePoolInstanceResponseRequired {
    /// Unique Oracle-assigned identifier for the request. If you need to contact Oracle about a particular request, please provide the request ID.
    pub opc_request_id: String,

    /// For optimistic concurrency control. See {@code if-match}.
    pub etag: String,

    /// The [OCID](https://docs.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the work request. Use [GetWorkRequest](https://docs.oracle.com/iaas/api/#/en/workrequests/latest/WorkRequest/GetWorkRequest) with this ID to track the status of the request.
    pub opc_work_request_id: String,

    /// Location of the resource.
    pub location: String,

    /// The returned model.InstancePoolInstance instance.
    pub instance_pool_instance: InstancePoolInstance,
}

impl AttachInstancePoolInstanceResponse {
    /// Create a new AttachInstancePoolInstanceResponse with required fields
    pub fn new(required: AttachInstancePoolInstanceResponseRequired) -> Self {
        Self {
            opc_request_id: required.opc_request_id,

            etag: required.etag,

            opc_work_request_id: required.opc_work_request_id,

            location: required.location,

            instance_pool_instance: required.instance_pool_instance,
}
    }

    /// Set opc_request_id
    pub fn set_opc_request_id(mut self, value: String) -> Self {
        self.opc_request_id = value;
        self
    }

    /// Set etag
    pub fn set_etag(mut self, value: String) -> Self {
        self.etag = value;
        self
    }

    /// Set opc_work_request_id
    pub fn set_opc_work_request_id(mut self, value: String) -> Self {
        self.opc_work_request_id = value;
        self
    }

    /// Set location
    pub fn set_location(mut self, value: String) -> Self {
        self.location = value;
        self
    }

    /// Set instance_pool_instance
    pub fn set_instance_pool_instance(mut self, value: InstancePoolInstance) -> Self {
        self.instance_pool_instance = value;
        self
    }
}


