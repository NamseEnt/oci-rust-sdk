...
pub struct GetAllDrgAttachmentsResponse {
    ...
    pub items: Vec<DrgAttachmentInfo>,
}

/// Required fields for GetAllDrgAttachmentsResponse
pub struct GetAllDrgAttachmentsResponseRequired {
    /// For list pagination. When this header appears in the response, additional pages of results remain. For important details about how pagination works, see [List Pagination](https://docs.oracle.com/iaas/Content/API/Concepts/usingapi.htm#nine).
    pub opc_next_page: String,
...
impl GetAllDrgAttachmentsResponse {
    /// Create a new GetAllDrgAttachmentsResponse with required fields
    pub fn new(required: GetAllDrgAttachmentsResponseRequired) -> Self {
        Self {
    ...
    /// Set items
    pub fn set_items(mut self, value: Vec<DrgAttachmentInfo>) -> Self {
        self.items = value;
        self
    }
}
